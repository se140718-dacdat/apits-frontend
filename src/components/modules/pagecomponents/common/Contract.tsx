...
import { getApply } from '../../../../redux/apiRequest';
...
const Contract: FC<Props> = ({ applyId }) => {
    const [apply, setApply] = useState<ApplyEntity>();
    ...
    const fetchData = async () => {
        setApply(await getApply(applyId));
        console.log(getApply(applyId))
    }
                    ...
                    ? (
                        <div className="pages">
                            <div className="page-size page__1">
                                ...
                                <h5>Bên tìm kiếm nhân sự (bên B): <strong style={{ textDecoration: "none" }}>{apply?.recruitmentRequest.creator.name}</strong></h5>
                                <p>Địa chỉ: {apply?.recruitmentRequest.workLocation}</p>
                                <p>Số điện thoại: {apply?.recruitmentRequest.hrPhone}</p>
                                <p>Email: {apply?.recruitmentRequest.hrEmail}</p>

                                <h5>Bên nhân sự (bên C): <strong>{apply?.candidate.name}</strong></h5>
                                <p>Địa chỉ: {apply?.candidate.address}</p>
                                <p>Số điện thoại: {apply?.candidate.phone}</p>
                                <p>Email: {apply?.candidate.email}</p>
                                ...
                                <p>Bên A và B đã thống nhất về mức phí dịch vụ dựa trên kinh nghiệm của ứng viên được cung cấp. Theo đó, với ứng viên có kinh nghiệm <strong>{apply?.recruitmentRequest.specialtyExperience.experience.name}</strong>, mức phí dịch vụ là <strong>{getCommissionPercent(apply?.recruitmentRequest.specialtyExperience.experience.name)}</strong> của mức lương cơ bản mà doanh nghiệp đã đề xuất là <strong>{currencyMaskString(apply?.recruitmentRequest.salaryDetail)}</strong>
                                ...
                                <p>Các Bên theo đây đồng ý rằng Bên B sẽ trả cho Bên C mức lương cơ bản là <strong>{currencyMaskString(apply?.recruitmentRequest.salaryDetail)} (VNĐ)</strong>, phải trả một tháng một lần và chịu các khoản khấu trừ và giữ lại thường xuyên theo yêu cầu của pháp luật.</p>
                                ...
                                <p>- Các Bên theo đây đồng ý rằng Bên C sẽ nhận được các phúc lợi (Bảo hiểm, Ngày lễ và Kỳ nghỉ) do Bên B cung cấp như được nêu dưới đây:<br />
                                    <ul>
                                        {apply.recruitmentRequest.benefits.split("\n").map((str) =>
                                            <li>{str}</li>
                                        )}
                                    </ul>
                                </p>
                                ...
                                <p>- Bên C đồng ý sẽ làm việc từ thứ 2 đến thứ 6, nghỉ trưa 1 tiếng.<br />
                                    - Cụ thể, Bên C đồng ý sẽ làm việc <strong>{apply.recruitmentRequest.typeOfWork}</strong>.<br />
                                    - Địa điểm làm việc của Bên C sẽ được đặt tại <strong>{apply.recruitmentRequest.workLocation}</strong> hoặc địa điểm khác mà các Bên có thể thỏa thuận tùy từng thời điểm.<br />
                                </p>
